...
class TestAnnotationParameters(object):
    ...
    def testOnGetParameters(self, server, fsAssetstore, admin):
        self.init(admin)
        ...
        resp = server.request(
            path='/RNAScope/annotation/parameters/' + str(annotation['_id']), user=admin)
        assert utilities.respStatus(resp) == 200
        parameters = resp.json['parameters']
        assert parameters['pixelThreshold'] == 1000
        assert parameters['pixelsPerVirion'] == 25
        assert parameters['roundnessThreshold'] == 0.5
